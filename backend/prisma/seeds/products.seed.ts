...
export async function seedProducts(
  ...
  for (const category of categoryKeys) {
    const pool = CATEGORY_IMAGE_POOL[category as keyof typeof CATEGORY_IMAGE_POOL] ?? [];
    for (let i = 1; i <= EXTRA_PRODUCTS_PER_CATEGORY; i++) {
      const primaryImage = pool[i % pool.length] ?? `https://picsum.photos/seed/${category.replace(/\s+/g, '')}${i}/600/600`;
